...
import { parseTime, uppercaseFirst } from '@/utils';
import { Delete, Edit } from '@element-plus/icons';
import { Column, RowTable, TemplateFilter } from '@/components/ManagementTable/type';
...
table.value.actions.router = table.value.actions.router || `${uppercaseFirst(camelCase(tableName))}Edit`;
...
const DeleteComponent = (row: { id: number | string }, $index: any) => (
  <el-link
  ...
  <>
    ...
    {(table.value.filters.templates || []).length > 0 && (
      <el-row gutter={20} type='flex' justify='space-between' class='tw-mb-6 tw-flex-wrap'>
        {(table.value.filters.templates || []).map((filter: TemplateFilter) => {
    ...
    <el-row gutter={20}>
      <el-col span={24}>
        <el-table
          ...
          {...(table.value.props || {})}
        >
          {table.value.columns.filter(Boolean).map((item: Column) => {
            return (
              <el-table-column
                key={item.field}
                prop={item.field}
              ...
              >
                {{
                  default: ({ row, column, $index }: RowTable<any>) =>
